...
parser.add_argument('--tf-embedding-type', type=str, default='static',
...
parser.add_argument('--tf-dataset', type=str, default='reuters_multilabel_dataset',
                    help='Options: reuters_multilabel_dataset, MR_dataset, SST_dataset')

parser.add_argument('--tf-loss-type', type=str, default='margin_loss',
                    help='margin_loss, spread_loss, cross_entropy')

parser.add_argument('--tf-model-type', type=str, default='capsule-A',
                    help='CNN, KIMCNN, capsule-A, capsule-B')

parser.add_argument('--tf-has-test', type=int, default=1, help='If data has test, we use it. Otherwise, we use CV on folds')    
parser.add_argument('--tf-has-dev', type=int, default=1, help='If data has dev, we use it, otherwise we split from train')    

parser.add_argument('--tf-num-epochs', type=int, default=20, help='Number of training epochs')
parser.add_argument('--tf-batch-size', type=int, default=25, help='Batch size for training')

parser.add_argument('--tf-use-orphan', type=bool, default='True', help='Add orphan capsule or not')
parser.add_argument('--tf-use-leaky', type=bool, default='False', help='Use leaky-softmax or not')
parser.add_argument('--tf-learning-rate', type=float, default=0.001, help='learning rate for training')#CNN 0.0005 
parser.add_argument('--tf-margin', type=float, default=0.2, help='the initial value for spread loss')
...
train, train_label, test, test_label, dev, dev_label, w2v= load_data(args.tf_dataset)    
...
X = tf.placeholder(tf.int32, [args.tf_batch_size, args.max_sent], name="input_x")
y = tf.placeholder(tf.int64, [args.tf_batch_size, args.num_classes], name="input_y")
...
if args.tf_embedding_type == 'rand':
...
if args.tf_embedding_type == 'static':
    W1 = tf.Variable(w2v, trainable = False)
    X_embedding = tf.nn.embedding_lookup(W1, X)
    X_embedding = X_embedding[...,tf.newaxis] 
if args.tf_embedding_type == 'nonstatic':
...
n_iterations_per_epoch = len(train) // args.tf_batch_size
n_iterations_test = len(test) // args.tf_batch_size
n_iterations_dev = len(dev) // args.tf_batch_size    

mr_train = BatchGenerator(train,train_label, args.tf_batch_size, 0)    
mr_dev = BatchGenerator(dev,dev_label, args.tf_batch_size, 0)
mr_test = BatchGenerator(test,test_label, args.tf_batch_size, 0, is_shuffle=False)
...
m = args.tf_margin
for epoch in range(args.tf_num_epochs):
    for iteration in range(1, n_iterations_per_epoch + 1):                
        ...
        #logging.info("EPOCH[{0}] train-loss={1}".format(iteration, loss_train))                     
    loss_vals, acc_vals = [], []
    ...
    logging.info("EPOCH[{0}] val-loss={1:.4f}".format(epoch+1, loss_val)) 
    logging.info("EPOCH[{0}] val-accuracy={1:.3f}".format(epoch+1, acc_val))                  
    ...
    logging.info("EPOCH[{0}] exact-match-rate={1:.3f}".format(epoch, acc))   
    logging.info("EPOCH[{0}] precision={1:.3f}".format(epoch, precision))   
    logging.info("EPOCH[{0}] recall={1:.3f}".format(epoch, recall))
    logging.info("EPOCH[{0}] f1-score={1:.3f}".format(epoch, F1))   

    #print ('\rER: %.3f' % acc, 'Precision: %.3f' % precision, 'Recall: %.3f' % recall, 'F1: %.3f' % F1)  
    if args.tf_model_type == 'CNN' or args.tf_model_type == 'KIMCNN':
        lr = max(1e-6, lr * 0.8)
    if args.tf_loss_type == 'margin_loss':    
        m = min(0.9, m + 0.1)
